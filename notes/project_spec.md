...
Status: living specification, version 3.6 after Task 30 substantive paper draft
Created: 2026-07-10  
...
The project target is fixed as a finite-stage logic of scoped, defeasible reliance on models under open-ended succession. Checkpoint D confirms a publication contract with four formal headline clusters, one mixed empirical result, one bounded optional policy/value case, and one running succession example. Task 22B completed the narrow operational policy/value pre-draft gate without adding an empirical claim or making the core calculus depend on the bridge. Tasks 26--30 provide the complete substantive paper draft; Task 31 confirms its mathematical, numerical, provenance, and citation integrity; and Task 31A confirms that the same supported content forms a consumable first-reader narrative. The logic formalizes licenses, local consequence, revision, overlap, fallback, and abstention; the neural work distinguishes representation, learning, calibration, evidence, masking, and usefulness; and the philosophical discussion remains neutral on whether truth is reducible to usefulness or true utility exists. Checkpoint E is next.
