...
Status: living specification, version 3.4 after Task 28 theorem draft
Created: 2026-07-10  
...
The project target is fixed as a finite-stage logic of scoped, defeasible reliance on models under open-ended succession. Checkpoint D confirms a publication contract with four formal headline clusters, one mixed empirical result, one bounded optional policy/value case, and one running succession example. Task 22B completed the narrow operational policy/value pre-draft gate without adding an empirical claim or making the core calculus depend on the bridge. Tasks 26--28 now give the public paper its reader-facing answer, tolerance derivation, running succession decision, compact semantics, H1--H3 theorem spine, empirical asymmetry, and related-work boundaries. The logic formalizes licenses, local consequence, revision, overlap, fallback, and abstention; the neural work distinguishes representation, learning, calibration, and usefulness; and the philosophical discussion remains neutral on whether truth is reducible to usefulness or true utility exists. Task 29 is next.
