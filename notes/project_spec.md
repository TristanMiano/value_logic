...
Status: living specification, version 3.0 after Checkpoint D pre-draft review
Created: 2026-07-10  
...
136. One bounded pre-draft gate remains. Task 22B must prove or constructively narrow a distribution-scoped behavioral reconstruction theorem for finite action scores. With `rho>=0`, coordinate error `e(s)=max_a|W_hat-W|`, intended-policy gap `g(s)`, and `D_mu(pi,pi_hat)=mu{pi_hat!=pi}`, the target raw oracle bound is `D_mu(pi,pi_hat) <= mu(e>rho)+mu(g<=2rho)` under a transparent decoder and compatible ties. The gap is `+infinity` at a forced singleton-action state. A certified risk statement additionally needs accepted bounds on the two event masses. This target is provisional until Task 22B completes its proof and fixtures.
137. Raw decoding stability and conservative certification are separate interfaces. Coordinatewise radius `rho` supplies the valid generic certified pairwise-gap radius `r_gap:=2rho`; an estimated winner gap above a certified `r_gap` soundly identifies its winner. The generic sufficient condition guaranteeing both recovery and non-abstention from the true gap can therefore be `g>2r_gap=4rho`. A separately accepted direct gap-error certificate may supply a tighter valid radius. Neither condition alone is a full `P_surrogate-rely` license.
...
139. Training-set action agreement has no automatic off-support consequence. Held-out IID disagreement, sequential trajectory similarity, and shared generalization mechanism are three different claims with different assumptions. Task 22B may prove a held-out concentration statement under a named distribution; “generalizes the same way” remains a future empirical companion question.
...
The project target is fixed as a finite-stage logic of scoped, defeasible reliance on models under open-ended succession. Checkpoint D confirms a publication contract with four formal headline clusters, one mixed empirical result, one bounded optional policy/value case, and one running succession example. The logic formalizes licenses, local consequence, revision, overlap, fallback, and abstention; the neural work distinguishes representation, learning, calibration, and usefulness; and the philosophical discussion remains neutral on whether truth is reducible to usefulness or true utility exists. One narrow operational policy/value theorem is the remaining pre-draft gate; it adds no empirical claim and leaves the core calculus independent of the bridge.
