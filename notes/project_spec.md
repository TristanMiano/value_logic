...
Status: living specification, version 3.1 after Task 22B bridge theorem
Created: 2026-07-10  
...
142. The repository's public workflow installs the experiment's pinned NumPy and CPU PyTorch dependencies plus its pinned Matplotlib analysis/import dependency before running `python -m verification`. Local success and public CI are distinct observations: the repaired public result remains pending until an authorized push, after which its exact run must be checked.
