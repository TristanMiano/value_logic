...
154. The Task 32 `/markdown` API validation established Markdown parsing and math-element parity but did not execute GitHub's browser-side MathJax macro allowlist. Post-push screenshots exposed the documented rejection of `\operatorname`. All 20 paper occurrences are replaced by the issue-recorded spacing-preserving `\mathop{\text{...}}` workaround, and a new regression requires zero `\operatorname`; the suite now passes 178/178. This is a typography-only erratum; it changes no mathematical object, theorem, evidence grade, frozen result, or policy/value boundary. Live browser confirmation remains a post-push observation.
