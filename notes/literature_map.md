...
## 19. Task 14C programs/proofs/certificates amendment

Task 14C verified seven primary sources before fixing the proof-carrying-plan claims:

| source | verified role | transfer boundary |
|---|---|---|
| `Howard1980` | formulae-as-types/proof-term construction | applies inside a named formal theory; it does not turn a neural score or empirical interval into a world-factive proof |
| `MartinLof1982` | constructive type theory as a language of mathematics and programming | supplies the constructive-programming precedent, not the project's empirical certificate semantics |
| `Hoare1969` | axiomatic rules for proving program properties | local/compositional program reasoning is established machinery; no general Hoare-logic novelty is claimed |
| `FreemanPfenning1991` | refinement of an underlying typed program language by more precise predicates | project grades/certificates become refinement types only after an exact encoding and checking theorem |
| `Atkey2018` | quantitative type theory tracking resource usage | motivates typed resource grades; it does not validate the project's particular risk or cost transformers |
| `Necula1997` | proof-carrying code checked against a consumer policy | formal PCC safety proofs are narrower and more deductive than the project's mixed formal/empirical certificate modes |
| `McConnellEtAl2011` | algorithms returning outputs with separately checkable witnesses | closest output-plus-certificate precedent; profile-indexed `K_3` licensing and world/state separation remain project integrations |

The primary sources force a conservative novelty statement. Structural induction, proof erasure, program assertions, refinement, resource tracking, and output-plus-certificate designs are prior art. [`formalism/08c_proof_carrying_plans.md`](../formalism/08c_proof_carrying_plans.md) contributes a compact integration contract: a finite plan jointly transforms payload, typed quantitative grade, and certificate/provenance; a checked root claim feeds the existing defeasible license calculus; and grounded/ranked assessment permits a frozen value-logic implementation to be evaluated without letting its own grant serve as its sole evidence.
