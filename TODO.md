...
**Next task: Task 14B — Repair and instantiate the theorem spine.**
...
- [ ] **Task 15 — Specify operational encodings for cases, domains, and models.**
  ...
  Create `ml/01_encodings.md`. Encode the surviving core before optional extensions and specify a **query-family-specific, architecture-neutral neural/symbolic interface** before choosing a scorer. Begin from atom addresses and the Task 14B query footprints: define dependency-scoped inputs `x(s,e,q,a)` and keep `P` primarily as the consumer that selects/aggregates atom records rather than a hidden cause of atom value. Specify which request/model/domain/evidence fields are explicit typed inputs, mechanically fixed context, learned sufficient statistics, validity/missingness indicators, external registry/provenance pointers, or forbidden compression targets. Separate the status-minimal `WF` plus `V/~_F` code from the stronger audit-preserving interface containing atom identity, diagnostics, safety projections, and future-profile information. State explicit invariances under provenance-ID renaming, candidate permutation where appropriate, and finite plan-DAG isomorphism. Do not turn a discrete-code bit bound into a neural-width bound.

  Compare a fixed finite model-indexed baseline with a candidate-conditioned shared scorer over a variable library; analyze sparse/on-demand pair comparisons versus quadratic matrices. Use flattened finite plan identity as the baseline and make a graph/set encoding conditional on a specific generalization or transparency hypothesis; retain loss-estimator identity and provenance either way. Divide mandatory core outputs from theorem-indexed optional routing, bridge, and plan-composition quantities so one network is not required to predict every extension field. Use record-derived certificate/loss regions and learned factorization as main options, with physics coordinates only as optional case-specific features. State identifiability/equivalence classes and interpretation tests; do not treat latent coordinates as ontology. End by checking that the encoding can state, without silently assuming, every Task 17 theorem target frozen at Checkpoint B.

- [ ] **Task 16 — Derive the ReLU license architecture.**

  Create `ml/02_relu_architecture.md`. Define a hybrid ReLU **reference** architecture whose learned part predicts continuous atom sufficient statistics—preferably certificate regions or dual support/refutation margins plus calibrated error/validity—while exact metadata, mechanically decidable `WF`, profile roles, `K_3` aggregation, active-set masking, and fallback remain symbolic. Explain why ReLU is analytically convenient—affine margins, exact finite CPWL constructions, explicit threshold statistics, and inspectable activation complexes—without claiming it is uniquely compatible or empirically ideal. Preserve inclusive-zero boundary conventions; rectified channels alone cannot distinguish boundary support from open/failure.

  Do not train an aggregate-status head that can contradict its atoms or a flat reason-code classifier. Preserve indexed diagnostics, safety projections, explicit invalid/missing/conflict states, exact scope/mode, registry/provenance and loss-estimator pointers, and direct-composite or Task 14A propagation certificates. Keep licensing separate from candidate selection/routing, and prohibit selection outside the derived active mask. Prefer one parameterized atom scorer `f(enc(s,e,q,a))` with address/type conditioning, comparing it against separate heads only as a design/ablation question. Provide a fixed finite-library ReLU baseline and a candidate-conditioned shared ReLU scorer with sparse pair evaluation and expandable external memory. Analyze—but do not yet commit to training—a monotone/lattice variant for proved monotone coordinates and a hard-MoE router for seam mismatch. Graph recursion and full comparison/bridge matrices remain conditional extensions.

- [ ] **Task 17 — State and verify the ReLU/atlas representation results.**

  Create `ml/03_representation_theorems.md`. Prove or sharply delimit the five targets frozen at Checkpoint B: (1) architecture-neutral exact factorization through the required `WF` observations and query quotient `V/~_F`, with a finer contract for diagnostics; (2) margin-robust approximate realization, where uniform statistic error plus boundary separation yields exact decoding and the uncertainty band becomes open; (3) exact finite ReLU computation of fixed-dimensional CPWL atom statistics followed by an external boundary-aware `WF + K_3` decoder, with verified primary theorems, size/depth, domain-extension, and architecture conventions; (4) the hard-routing seam characterization on a conforming finite polyhedral complex, using agreement on every relevant common face/intersection, with discontinuous mismatch impossible for one ordinary continuous ReLU output but representable by an external hard router; and (5) the limitation separating fixed model-indexed outputs from candidate-conditioned scoring over an expandable registry.

  Make explicit that a finite discrete-code bit lower bound is not a real-valued output-width lower bound, that `Ill/Well` is a canonical decodable normal form rather than a required internal datatype, and that direct discrete `K_3` output is supplied by the external decoder rather than continuity of the ReLU map. Connect the construction to scalar certificate-region margins and to the Task 14B dependency interface. State exactly which results are ReLU/CPWL-specific and which transfer to monotone/lattice or hard-MoE families. Universal approximation alone does not establish semantic compatibility, efficiency, learnability, calibration, or interpretability. Keep scientific-cover, router-partition, and activation-complex alignment empirical.

- [ ] **Task 18 — Decide the training objective.**

  Create `ml/04_losses.md`. Analyze certificate-region/risk regression, calibrated prediction error, positive/countercertificate evidence, learned validity where needed, open/missingness obstacles, auxiliary atom-state losses, hinge/ranking losses for comparison/selection, calibration/selective-risk objectives, explicit reject options, and plain softmax cross-entropy. Keep three levels separate: the task/value criterion `L_q`, any learned model that estimates consequences or `L_q`, and the optimization loss used to train that estimator or license scorer. Choose one structured primary objective that learns sufficient statistics and uncertainty/calibration, plus one deliberately simpler classification baseline. Derive atom states and `Granted/Refused/Withheld/Undefined` symbolically through the fixed decoder; direct atom/status cross-entropy may be auxiliary, and an independent aggregate or flat reason classifier only an ablation. Keep router loss separate from license/evidence loss. Explain simultaneous/empty usable sets, comparison-driven changes without conflating them with evidence failure, conservative open bands under approximation error, and risk–coverage calibration.

- [ ] **Checkpoint C — Reassess the roadmap after the neural representation and objective.**

  Create `notes/checkpoints/C_neural_blueprint.md` and apply the checkpoint protocol. Check that the proposed encodings, hybrid ReLU reference architecture, representation results, and training objective form one implementable and falsifiable system under the Task 14B dependency contract. Select at most one trained non-ReLU comparison unless two distinct hypotheses are adequately powered. The leading candidates are a monotone/lattice model for proved tolerance/refinement monotonicities and a hard MoE for seam discontinuity; graph/set models require Task 15 evidence that flattening or shared candidate scoring is inadequate. Specify matched semantic outputs, capacity/compute reporting, and fairness protocol; if no comparison is feasible or informative, retain ReLU alone. Revise Task 19 onward and add only necessary baseline, calibration, capacity, or impossibility work before implementation.

- [ ] **Task 19 — Design the decisive toy experiment.**

  Create `experiments/01_design.md`. Specify a synthetic theory-succession generator whose domains, atom sufficient statistics, target semantics, and update footprints are fixed independently of the trained network. Use compact atom-indexed witnesses/obstacles rather than a flat reason vocabulary, and organize the benchmark in powered tiers. The **core succession tier** must include external and fallback-induced thresholds, all four derived outcomes, missing/invalid evidence, several simultaneous licenses, a gap, lapse versus rebuttal, irrelevant versus relevant updates, and a later-added dominator while separating continued use, nonselection, and archive retention. The **routing tier** must include matched continuous-seam and discontinuous-seam cases, overlap, fallback, selected-scope shift, and severity-aware risk. An optional **composition stress tier** may include extensionally equal but cost-different plans, a fallible loss estimator, bridge uncertainty, or failed splitting only if it does not underpower the core claims. Use the ReLU reference and only the comparator selected at Checkpoint C under matched semantic outputs, capacity/compute reporting, and an architecture-specific hypothesis. Define datasets, splits, metrics, ablations, expected plots, falsification criteria, and seeds. Treat quotient/atom fidelity, calibration, and risk–coverage as primary; update/routing as secondary; and activation/domain alignment as exploratory.

- [ ] **Task 20 — Implement the experiment.**

  Add a small, readable implementation under `experiments/` with configuration, tests, and a reproducible entry point. Implement only the Checkpoint C blueprint. Include the basic ReLU reference MLP and any selected architecture comparator behind the same semantic output interface, structured atom objective, `WF + K_3` symbolic decoder, indexed witness/obstacle diagnostics without a closed reason enum, comparison baselines, simultaneous/empty usable sets, comparison/selection routing, fallback/abstention, signed and rectified margins, registry identifiers, machine-readable update traces, and activation-region inspection where meaningful for the architecture.

- [ ] **Task 21 — Run and analyze the experiment.**

  Create `experiments/02_results.md` plus machine-readable results and figures. Report the preregistered tier-specific endpoints. Primary results are sufficient-statistic/atom and query-quotient fidelity, calibrated boundary behavior, confusion among refuted/open/WF-failure cases, multi-label/empty-set fidelity, and risk–coverage/fallback harm. Secondary results are dependency-scoped update invariance, comparison/model-addition changes, old-model continued-use versus archive-only outcomes, routing/seam behavior, selected-scope risk, and any optional composition stress tests. Report architecture comparisons under the Checkpoint C fairness protocol. Activation/domain alignment and learned-coordinate interpretation remain exploratory. Keep functional fit, calibration, semantic alignment, architecture-specific geometry, and interpretability conclusions separate.
...
- [ ] **Task 24 — Build the counterexample and limitations section.**
  ...
  Create `notes/limitations.md`. Organize limitations by the failed assumption or theorem boundary that produces them rather than as a flat taxonomy. Cover underdetermination; scaled rewards/losses; policy/environment/return dependence; relative model granularity; component interaction and sensitivity; modeled-loss regress; cyclic self-reference without fixed-point semantics; architecture dependence and absence of ReLU uniqueness/optimality; universal-approximation insufficiency; comparator fairness; profile-dependent reliance/preference; certificate-mode mismatch; unknown candidates and finite search; pair/bridge scaling; rectification loss; hard-routing discontinuity; surrogate-without-mechanism; adversarial routing; forgetting; unsupported domains; ontology shift/nonstationarity; atlas-metaphor limits; activation/domain misalignment; and usefulness versus truth. Link every falsified/limited claim to its project-impact analysis.

- [ ] **Task 25 — Freeze the paper’s claims and outline.**

  Create `paper_outline.md`. Select only claims supported by completed proofs, verified literature, or experiment results. Freeze a **core contribution set** separately from standard formal integrations, optional extensions, empirical results, policy/value case study, and interpretive motivation. Separate the architecture-neutral semantic interface, ReLU reference construction, and comparative evidence; do not imply architectural uniqueness or optimality. Put the four opening design questions near the start and any surviving interpretability bridge after the neural/experimental results. Write a section-level outline with word budgets, theorem/evidence dependencies, notation cross-check, countermodel map, contribution list, and a cut list that removes historical record taxonomies, routine lemmas, and unused extension machinery from the main paper.
...
- [ ] **Task 26 — Draft the formal paper’s motivation and related work.**
  ...
  Begin `paper.md` with title, abstract, introduction, the derivation and limits of `Pi(M,D,epsilon)`, relative model granularity and finite recursive composition, the distinction between a loss criterion and a model that estimates it, the Task 11A judgment design and rejected alternatives, the representation question, licensed model covers versus ReLU activation complexes, motivating examples, contributions, scope, and related work. Preserve whatever distinctions Task 11A shows are necessary among truth, adequacy/usability, finite relative comparison, current use/selection, and retention without pretending the terminology was forced from the outset.

- [ ] **Task 27 — Draft the formal definitions and semantics sections.**

  Extend `paper.md` with the compact `E,Q,S` signature, semantic world index, finite typed atom family/refinement preorder, profiles, `WF + K_3` assessment, indexed diagnostic sum, consequence/update rules, canonical read/write locality interface, comparison/use relationship, and open-ended stage semantics. Explain finite plan-DAG elaboration and typed higher-order evaluator requests, distinguishing them from cyclic fixed points. Put detailed records and atlas/bridge/Pareto taxonomies in appendices or clearly marked extensions; use derived predicates and quotients where they preserve the required consumer observations.

- [ ] **Task 28 — Draft the theorem and proof sections.**

  Extend `paper.md` with only the settled theorem spine and representation results, including countermodels where stronger desired claims fail. Lead with the repaired profile, stability, update-locality, and Task 17 clusters. Present transport/routing as standard mathematics integrated with the calculus rather than an independent novelty claim. Organize around characterizations, impossibilities, quantitative bounds, and exact representation conditions; label imported/standard/definitional results honestly and move them to appendices when they obscure the paper-carrying results.

- [ ] **Task 29 — Draft the neural representation, learning, and interpretability sections.**

  Extend `paper.md` with theorem-indexed neural contracts: query-family quotient versus audit-preserving diagnostics, architecture-neutral atom statistics, uncertainty bands, symbolic status decoder, external registry/provenance and scaling assumptions, flattened versus conditional compositional encodings, target loss versus estimator versus training loss, ReLU reference results, any selected comparator, seam/hard-routing limits, semantic alignment metrics, objective choice, transparency grades, and exact limitations. State that discrete-code bits do not imply neural width. Include policy/value only at the evidence grade frozen by Tasks 22–25.

- [ ] **Task 30 — Draft the experiment, discussion, and conclusion.**

  Complete the substantive draft of `paper.md` with tiered results and ablations, four-way/status and risk–coverage failures, update/retention and seam behavior, scoped architecture comparisons, philosophical interpretation, interpretability implications, limitations, future work, and conclusion. Separate confirmatory functional/calibration endpoints from exploratory activation alignment. Do not let synthetic success imply scientific realism, mechanistic transparency, or ReLU optimality.

- [ ] **Task 31 — Perform the mathematical and citation audit.**

  Check every symbol, definition, theorem dependency, proof, numerical claim, and citation. Verify the compact/elaborated signature map, Task 14B read/write footprints and change-completeness, finite plan-composition typing, separation of criterion/estimator/training loss, and any fixed-point semantics. Audit query-family choice, canonical-tag claims, finite discrete-code bits versus neural width, inclusive boundary conventions, every universal-approximation appeal, and architecture comparisons so ReLU existence is not inflated into uniqueness, efficiency, learnability, calibration, or interpretability. Ensure no flat reason taxonomy or record schema is primitive without necessity; classify every result as definitional, standard, characterization, impossibility/countertheorem, quantitative bound, or conjecture. Confirm that at least three paper-carrying clusters survive. Require primary sources or repository proof/experiment for every technical claim and an authoritative source for retained physics. Update the ledger and verify every `X1` project-impact propagation into both public artifacts.
...
- [ ] **Task 33 — Produce the Substack adaptation.**
  ...
  Create `substack_post.txt` from the audited paper. Use plain prose, Unicode where helpful, very few display equations, no Markdown formatting, and concrete examples. Retain the four opening motivations, the ReLU-reference-not-uniqueness caveat, and all other necessary qualifications without introducing claims absent from the paper.
...
- **2026-07-12 — Checkpoint B clears the neural transition only after one focused repair.** Profile relative completeness is the cleanest current logic result; the stability package needs precision and classical positioning; and the update characterization needs a concrete atom-locality/change-completeness theorem. Task 14B is the only new pre-ML task, and broader metatheory is rejected unless a later consumer exposes a specific gap.
...
- **2026-07-12 — Architecture alternatives require distinct hypotheses.** Monotone/lattice models are nominated only for proved monotone coordinates, and hard MoE only for seam discontinuity/routing. The neuro-symbolic decoder is shared infrastructure; maxout is presently redundant; graph/set models wait for evidence that flattening or candidate-conditioned scoring is inadequate.
