...
**Next task: Task 11B — Mechanize and test the integrated finite witness.**
...
- at least one nontrivial result about supersession/retention is proved;
...
- [x] **Unscheduled Checkpoint A1 — Adjudicate the Fable 5 external audit.**
  ...
  Completed 2026-07-11. Created [`notes/checkpoints/A1_external_audit_response.md`](notes/checkpoints/A1_external_audit_response.md). Accepted and repaired C1–C10 with one bibliographic correction to the audit itself; introduced explicit world/stage semantics, substantive versus stage-bound request fields, typed action-authorizing roles, overlap-scope witness certificates, transparent joint-score certificates, `D1` design dispositions, and top-of-file Task 11A interface notices. Corrected F14's attribution to Arora et al. (2018), Theorem 2.1, added Wang–Sun (2005), and retained He et al. (2020) for detailed construction/size analysis. Added Task 11B for executable semantic verification, Task 12A for the missing core-related literature, and Task 22A for the recursive-judgment information claim; strengthened Tasks 12–18 and 25. The next task changed from Task 12 to Task 11B.

- [ ] **Task 11B — Mechanize and test the integrated finite witness.**

  Create a small standard-library Python reference implementation under `verification/` plus tests and a reproducible command. Encode typed world/stage records, the four statuses, lossless `Diag`, required/report-only atoms, canonical profiles, selectors, the three-stage Task 11A witness, comparison eligibility on the exact evaluation scope, profile-local update, lapse versus rebuttal, and provenance links. Derive the aggregate status symbolically from atom assessments rather than learning or independently storing it. Assert every table and transition in `formalism/05a_integration.md`, including the difference between marginal score intervals and tighter joint certificates. Add a lightweight repository check for broken local Markdown links and run both checks in a minimal GitHub Actions workflow. This is semantic verification infrastructure, not the neural experiment or proof-assistant formalization.

- [ ] **Task 12 — Analyze open-endedness and non-finality.**

  Create `formalism/06_open_endedness.md`. Starting from Task 11A's profile-indexed interface and the Task 11B executable reference, define stage/refinement DAGs over world/stage pairs, compatible continuations, live alternatives, current `Lic_P` grant, eventual profile-indexed stability, known/certified stability, semantic finality, and optional meta-level truth. Hold the request/profile fixed when asserting stability; a changed profile creates a new request. Give countermodels showing that extendability permits both eventual stabilization and endless change, that one profile can stabilize while another changes, and that stabilization need not have a knowable arrival time. Target a non-vacuous atom-level characterization: state which deterministic proof-backed, statistical/evidence-backed, and open-library comparison atoms can admit internal stability certificates under which continuation restrictions. In particular, test whether comparison atoms can be known-stable when `AddModel` plus valid evaluation remains an admissible continuation. Prove the strongest conditional non-certifiability result supported by explicit indistinguishable-continuation/live-alternative assumptions. Treat absence of `Final` from the base language as syntactic non-expressibility, not a substantive theorem. Include Löb/GL only if a genuine provability translation with derivability/fixed-point conditions is constructed; otherwise record a concise rejection, and do not use “anti-Löbian” as a theorem name.

- [ ] **Task 12A — Fill the core-related literature gaps before freezing the calculus.**

  Create `notes/literature_core_supplement.md` and update `references.bib`, `notes/literature_map.md`, and affected ledger rows using verified primary sources. Audit input/output logic for output-producing rather than truth-detaching consequence; prioritized reasons/defaults; justification and labelled deductive logics for explicit evidence/provenance terms; awareness/unawareness logics for library versus conceivable alternatives; Bochvar/strong-Kleene/logic-of-partial-functions precedents for the four-status aggregation; conformal prediction's primary sources; and high-confidence/safe policy improvement over a fallback. Determine exact structural similarities and mismatches without importing representation, completeness, or soundness theorems whose hypotheses are absent. Treat the Fable audit's L1–L5 list as search leads, not verified authority.

- [ ] **Task 13 — Select the smallest publishable logic and state its metatheory goals.**

  Create `formalism/07_core_calculus.md`. Consolidate Tasks 7–12A into the smallest publishable calculus: scalar typed risk/tolerance, abstract valid certificates, four statuses, mandatory profile-indexed `Lic_P`, finite required/report-only atoms, the named `P_rely` and comparative profiles as instances rather than separate primitives, labeled consequence, finite record/update, lapse versus rebuttal, profile-indexed active sets/gaps/fallback, finite relative comparison/preferred-use assessment, and extendable-stage semantics. Use explicit two-sorted models `<W,S>`: annotate every core judgment as world-level, stage-level, or mixed and state certificate-mode relations before any soundness theorem. Separate profile shape from atom parameters, define typed atom entailment/refinement, and lift it to a practically usable profile preorder so license grant is antitone under exact side conditions. Analyze the four-status aggregator as a finite algebra, preserve `Diag`, and settle how certified safety refusals remain visible when `Undefined` has aggregate precedence. Give syntax, models, satisfaction/assessment, proof rules, and dynamics in one place. Require every selector to declare its minimum profile and keep usability, current selection, and archival retention distinct. Explicitly classify vector/Pareto risk, distributional splitting, full bridge/atlas machinery, statistical certificate instantiations, merges, and policy/value work as extensions. Include a central notation/glossary index and a theorem-dependency graph separating definitions, algebraic corollaries, nontrivial candidate results, countermodels, and conjectures.

- [ ] **Task 14 — Prove or refute the core calculus results.**

  Create `formalism/08_metatheory.md`. Prove or refute the actual core rule package against the Task 13 `<W,S>` semantics, revising the calculus when necessary. At minimum settle mode-scoped rule soundness, status exclusivity/algebra, labeled non-explosion, tolerance monotonicity, profile-refinement antitonicity, safe fallback on gaps, raw heredity without current-status monotonicity, the exact effect of purely comparative defeat on every Task 11A profile, failure of unrestricted rational monotony, failure of global closure from finite search, and the strongest Task 12 atom-level stability/non-finality result. Replace the merely sufficient “conservative update” lemma with the strongest exact or near-exact persistence characterization supported by the dependency/impact semantics—for example, necessity and sufficiency in terms of required-atom impact cones and relevant comparison-scope expansion. Treat KLM premise postulates only as structural comparisons because the explicit-premise relation is monotone and the project's nonmonotonicity primarily lives in stage transitions. Distinguish definitional consequences and finite-algebra corollaries from substantive theorems; the final contribution must contain at least one genuinely nontrivial theorem or countertheorem. Treat Pareto/splitting/bridge results as extensions unless a nontrivial stronger theorem is proved. Every refutation must update the claim ledger with project impact.

- [ ] **Checkpoint B — Reassess the roadmap after the core calculus and metatheory.**

  Create `notes/checkpoints/B_core_metatheory.md` and apply the checkpoint protocol. Decide whether the proved, refuted, or weakened results still support the proposed ReLU correspondence; identify which formal objects and theorems actually need neural representation; and revise Task 15 onward so the ML work targets the surviving calculus rather than the earlier aspirations.
...
- [ ] **Task 16 — Derive the ReLU license architecture.**
  ...
  Create `ml/02_relu_architecture.md`. Define a basic ReLU reference architecture with candidatewise predicted risk/certificate-validity heads, signed hard/fallback/constraint/comparison margins, paired or exposed negative channels, multi-label usable/licensed sets under the Task 11A design, comparison and selection outputs, routing, and explicit fallback/abstention. Supervise component atoms and derive the top-level four-way status symbolically through the audited aggregation rule; do not train an independent aggregate-status head that can contradict its components. Preserve `Diag`, safety reasons, registry/provenance pointers, and missingness outside any scalar margin. Compare separate atom heads with a parameterized requirement-profile head rather than assuming the factorization in advance. Provide both a fixed finite-library MLP baseline and a shared scorer coupled to expandable external memory. Treat nested model-building choices, full bridge matrices, and mixtures as optional extensions.

- [ ] **Task 17 — State and verify the ReLU/atlas representation results.**

  Create `ml/03_representation_theorems.md`. Prove the required finite constructions or cite precise primary CPWL/ReLU theorems with architecture conventions. Settle exact versus approximate representation of scalar/vector core margins and four-way/multi-label outputs; open, closed, and equality threshold conventions; fixed finite versus expandable libraries; and the role of external argmax. Import Task 11's counterexample showing that discontinuous hard-routed expert outputs are not generally one ordinary continuous ReLU output, and state the seam-agreement/approximation alternatives. Keep scientific-cover, router-partition, and activation-complex alignment as a separate empirical claim.

- [ ] **Task 18 — Decide the training objective.**

  Create `ml/04_losses.md`. Analyze risk/log-loss regression, atom/certificate classification, binary or multi-label cross-entropy, derived four-way status, hinge/ranking losses for comparison/selection, calibration/selective-risk objectives, explicit reject options, and plain softmax multiclass cross-entropy. Choose one structured primary objective (possibly multitask) and one deliberately simpler comparison baseline. The primary objective must supervise required/report-only atoms, missingness, and safety reasons, then compute `Granted/Refused/Withheld/Undefined` symbolically; an independently learned aggregate may appear only as an ablation. Explain simultaneous/empty usable sets, comparison-driven changes without conflating them with evidence failure, and risk–coverage calibration.
...
- [ ] **Task 19 — Design the decisive toy experiment.**
  ...
  Create `experiments/01_design.md`. Specify a synthetic theory-succession generator whose domains and target semantics are fixed independently of the trained network. Include external and fallback-induced thresholds, several simultaneously adequate/usable models, a certified gap, unknown applicability, all four statuses, costs/coverage, later evidence causing lapse versus refusal, and a later-added model that changes comparison/selection while allowing Task 11A's chosen semantics to distinguish continued use, nonselection, and archival retention. Include exact/approximate/incompatible or unknown overlaps and at least one failed split or distribution-shift case. Define datasets, splits, metrics, ablations, expected plots, falsification criteria, and seeds. Measure active-set, routing, calibration, risk–coverage, update/retention, and activation/domain alignment separately.

- [ ] **Task 20 — Implement the experiment.**

  Add a small, readable implementation under `experiments/` with configuration, tests, and a reproducible entry point. Implement only the Checkpoint C blueprint. Include the basic ReLU MLP, structured objective, comparison baselines, four-way status, simultaneous/empty usable sets under the Task 11A interface, comparison/selection routing, fallback/abstention, signed and rectified margins, registry identifiers, machine-readable update traces, and activation-region inspection.

- [ ] **Task 21 — Run and analyze the experiment.**

  Create `experiments/02_results.md` plus machine-readable results and figures. Report component-risk and status calibration, confusion among refusal/withholding/undefined, multi-label usable-set fidelity under the chosen semantics, gap/unknown handling, risk–coverage and fallback cost, comparison/selection routing, model-addition supersession, old-model continued-use versus archive-only outcomes, tolerance monotonicity, subdomain retention, activation/domain alignment, and distribution-shift/identifiability failures. Keep functional fit, calibration, semantic alignment, and interpretability conclusions separate.
...
- [ ] **Task 27 — Draft the formal definitions and semantics sections.**
  ...
  Extend `paper.md` with the minimal core signature, the chosen assessment/license semantics, consequence/update rules, the chosen relationship between comparison and use status, and open-ended stage semantics. Present Pareto retention, splitting, and licensed-atlas/bridge machinery explicitly as extensions rather than silently enlarging the core.

- [ ] **Task 28 — Draft the theorem and proof sections.**

  Extend `paper.md` with only the settled metatheory and representation results, including countermodels where stronger desired claims fail. Move routine definitional sanity checks or long extension proofs to appendices when they obscure the nontrivial core results.

- [ ] **Task 29 — Draft the neural representation, learning, and interpretability sections.**

  Extend `paper.md` with information-preserving/lossy maps, structured status/margin outputs, external registry and scaling assumptions, ReLU architecture, finite representation and hard-routing limits, semantic alignment metrics, objective choice, transparency criteria, and exact limitations. Include the policy/value companion case only at the evidence grade and scope frozen by Tasks 22–25.
...
- **2026-07-11 — Neural work targets structured finite-stage outputs.** The reference architecture must preserve signed component margins, four operational statuses, simultaneous/empty usable sets, comparison and selection information according to Task 11A's chosen design, fallback, reason codes, and registry/provenance pointers. A fixed finite baseline and expandable external-memory design are both required.
