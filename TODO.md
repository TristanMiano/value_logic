...
**Next task: Task 12 — Analyze open-endedness and non-finality.**
...
- [x] **Task 11A — Resolve the finite-stage license/comparison interface and build an integrated witness model.**
  ...
  Completed 2026-07-11. Created [`formalism/05a_integration.md`](formalism/05a_integration.md). Compared layered, parameterized-profile, and strong-license interfaces without assuming a winner, then selected mandatory profile-indexed licensing `Lic_P` because it subsumes the other designs as named profiles, supports a formal profile-strength relation, and lets selectors declare their required authorization level. Defined finite required/report-only atoms; `P_rely`, `P_pref-rel`, and `P_pref-cert`; four-way atom/profile assessment; mandatory profile references; relative-undefeated versus certified-undominated behavior under unknown comparisons; profile-local updates; selector-required profiles; and profile-indexed current/stability views. Built a four-plan, three-stage witness with explicit fallback, overlap, gap, all four statuses, simultaneous usability, continued old-model use on an edge subdomain, usable-but-unselected and archive-only retention, model-addition supersession, unknown comparison, successful and withheld splits, five bridge statuses, lapse, rebuttal, and complete provenance. Proved nine structural results and mapped repairs into Tasks 8–14. Falsified inherited claim E05's universal closure clause while supporting the profile-indexed replacement and recording its project impact.

- [ ] **Task 12 — Analyze open-endedness and non-finality.**

  Create `formalism/06_open_endedness.md`. Starting from Task 11A's profile-indexed interface, define stage/refinement DAGs, compatible continuations, live alternatives, current `Lic_P` grant, eventual profile-indexed stability, known/certified stability, semantic finality, and optional meta-level truth. Give countermodels showing that extendability permits both eventual stabilization and endless change, that one profile can stabilize while another changes, and that stabilization need not have a knowable arrival time. Prove the strongest conditional non-certifiability result supported by explicit indistinguishable-continuation/live-alternative assumptions. Treat absence of `Final` from the base language as syntactic non-expressibility, not a substantive theorem. Include Löb/GL only if a genuine provability translation with derivability/fixed-point conditions is constructed; otherwise record a concise rejection, and do not use “anti-Löbian” as a theorem name.

- [ ] **Task 13 — Select the smallest publishable logic and state its metatheory goals.**

  Create `formalism/07_core_calculus.md`. Consolidate Tasks 7–12 into the smallest publishable calculus: scalar typed risk/tolerance, abstract valid certificates, four statuses, mandatory profile-indexed `Lic_P`, finite required/report-only atoms, the named `P_rely` and comparative profiles as instances rather than separate primitives, labeled consequence, finite record/update, lapse versus rebuttal, profile-indexed active sets/gaps/fallback, finite relative comparison/preferred-use assessment, and extendable-stage semantics. Give syntax, models, satisfaction/assessment, proof rules, and dynamics in one place. Require every selector to declare its minimum profile and keep usability, current selection, and archival retention distinct. Explicitly classify vector/Pareto risk, distributional splitting, full bridge/atlas machinery, statistical certificate instantiations, merges, and policy/value work as extensions. Include a theorem-dependency graph separating definitions, elementary lemmas, nontrivial candidate results, countermodels, and conjectures.

- [ ] **Task 14 — Prove or refute the core calculus results.**

  Create `formalism/08_metatheory.md`. Prove or refute the actual core rule package against the Task 13 semantics, revising the calculus when necessary. At minimum settle rule soundness, status exclusivity, labeled non-explosion, tolerance monotonicity, safe fallback on gaps, raw heredity without current-status monotonicity, conservative-update persistence, the exact effect of purely comparative defeat on every judgment chosen in Task 11A, failure of unrestricted rational monotony, failure of global closure from finite search, and the strongest Task 12 non-finality result. Distinguish definitional consequences from substantive theorems. Treat Pareto/splitting/bridge results as extensions unless a nontrivial stronger theorem is proved. Every refutation must update the claim ledger with project impact.
...
- [ ] **Task 18 — Decide the training objective.**
  ...
  Create `ml/04_losses.md`. Analyze risk/log-loss regression, certificate/status classification, binary or multi-label cross-entropy, four-way classification, hinge/ranking losses for comparison/selection, calibration/selective-risk objectives, explicit reject options, and plain softmax multiclass cross-entropy. Choose one structured primary objective (possibly multitask) and one deliberately simpler comparison baseline. Explain how the objective supervises `Granted/Refused/Withheld/Undefined`, simultaneous/empty usable sets under the chosen Task 11A semantics, comparison-driven changes without conflating them with evidence failure, and risk–coverage calibration.
...
- **2026-07-11 — Universal license closure is falsified.** E05 is false when read as “every agent-level license includes a no-better-model clause.” The supported replacement is that profiles authorizing comparative preferred use must name finite search/closure strength. This is an interface correction, not a threat to the project goal.
