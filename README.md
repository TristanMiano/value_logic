...
Tasks 0–14, Checkpoint A, and the unscheduled external-audit Checkpoint A1 are complete. The finite-stage licensing interface, consequence and update rules, dominance and retention distinctions, overlapping scientific-model cover, bridge taxonomy, and a repaired integrated finite witness have been developed. The compact `WF + K_3` semantics and the complete finite witness are executable and tested with `python -m verification`. Continuation semantics separates current grants, eventual stability, certified stability, semantic finality, and optional truth. The core-related literature has been audited against primary sources with explicit theorem-transfer boundaries. The compact core calculus fixes the canonical request `(s,e,q,P)` and proves each coordinate indispensable. Its metatheory now includes robust update persistence, sound and relatively complete profile refinement on the independent fragment, the query-induced diagnostic quotient, and explicit countermodels to stronger factivity, closure, fallback, persistence, and composition claims.

The current next step is Task 14A: proving the domain-transport and routed-cover bounds.
