...
[`Task 22A`](formalism/09_judgment_information.md) proves a scoped version of the recursive-judgment information claim. Positive population strict-proper-score improvement over the true nuisance-conditioned Bayes baseline implies positive `I(R;Y|N)` for a pre-outcome report `R`; a `delta`-nat log-loss gain gives `I(R;Y|N)>=delta`. Under explicit mediation, the bound transfers to the outcome-identifiable task quotient. Non-Bayes comparators, omitted nuisance, leakage, duplicated task types, instability, calibration alone, and copying delimit the result.
...
[`Task 26`](paper.md) begins the formal paper with its title, abstract, reader-facing introduction, one synthetic succession decision, and related work by claim boundary. Its opening preserves the author's originating philosophical motivation: Newton's laws are superseded and “falsified” in an unrestricted sense while remaining taught and used; present physics is expected to be superseded in turn; and the chain has no known endpoint. It derives `epsilon` from both external requirements and the declared fallback before neural notation, separates plan granularity and modeled loss, states the profile-indexed judgment as a design choice, and places the complete frozen transfer-versus-coverage asymmetry beside the ReLU motivation. The optional policy/value bridge is foreshadowed while retaining the exact finite encoder-image existence claim and true-utility neutrality.

[`Task 27`](paper.md#3-a-compact-finite-stage-license-calculus) adds the compact `E,Q,S` semantics with world index `W`, typed profiles and refinement, `WF + K_3`, complete diagnostics, labelled use, selection/fallback, history-preserving update, and fixed-query continuation stages. It keeps object outputs, internal derivations, external metatheorems, and accepted certificate judgments distinct, and introduces `(payload, grade, evidence)` only as the interface needed for the later composition result. The abstract and introduction now make the philosophical transition explicit: a successor may be more accurate or general while an adequate predecessor retains comparative value on a bounded task because it is cheaper or easier to compute, inspect, or deploy. This task-relative value does not define truth.

[`Task 28`](paper.md#4-open-succession-and-local-revision) adds the H1--H3 theorem spine and [Appendices A--C](paper.md#appendix-a-core-syntax-and-elaboration-reference). The paper now gives the exact scope and constructive boundaries for profile refinement, deterministic/statistical/open-library stability, typed diagnostic locality and update change-completeness, finite proof-carrying composition, grounded/ranked assessment, routed risk, and path-sensitive error propagation. The running succession explains each result, including expiry, rebuttal, irrelevant updates, later domination, selected-subdomain risk, and fallback severity. Established sequential, database, program-logic, proof-carrying, and fixed-point patterns are identified as scoped integrations.

[`Task 29`](paper.md#6-architecture-neutral-representation-and-a-relu-reference) adds H4, the learning/masking interface, and [Appendix D](paper.md#appendix-d-representation-proofs-and-boundary-constructions). Consumer-relative status and audit factorization precede the finite CPWL/ReLU reference witness; accepted uncertainty bands, exact evidence and state, registry/provenance, payload/grade/certificate separation, flat/DAG records, hard seams, variable libraries, boundary collisions, and scale covariance delimit the result. The canonical `A and I and C` calculation separates arbitrary positivity, predicted slack, accepted atom surplus, full license, and active-set ranking, while a bias/bypass fixture shows why zero alone cannot quarantine computation. The optional [policy/value bridge](paper.md#8-optional-policyvalue-and-recursive-judgment-bridge) and [Appendix F](paper.md#appendix-f-policyvalue-information-and-transparency-details) preserve the exact encoder-image isomorphism/existence result, add the raw and evidence-certified behavioral bound with its separate conservative non-abstention condition, retain true-utility neutrality and transparent-harness assumptions, and report interpretability as seven independent evidence axes.

[`Task 30`](paper.md#7-a-frozen-synthetic-test-of-reusable-statistics-and-licensed-coverage) completes the substantive paper draft with the frozen experiment, [discussion and prospective work](paper.md#10-discussion-limitations-and-future-work), [conclusion](paper.md#11-conclusion), and [Appendix E](paper.md#appendix-e-frozen-experiment-complete-results-and-reproducibility). The empirical section leads with decisive opposing effects: reusable-statistic transfer is supported, boundary superiority and in-regime noninferiority are refuted at their registered margins, and marginal proposal coverage passes while misses and `0.9962` fallback mass make usable coverage poor. It discloses the unmatched-coverage drift, unavailable trace secondaries, artifact-transport erratum, omitted architecture comparison, and deterministic-only system witness. Functional fidelity, calibration, certificate validity, routed risk, activation alignment, and policy/value evidence remain separate.

[`Task 31`](notes/mathematical_citation_audit.md) completes the mathematical,
numerical, provenance, and citation audit. All four formal paper-carrying
clusters survive; the finite policy/value encoder-image existence result is
preserved at its stated scope; `F35a--c/F36` and the C20 hash chain recompute
exactly. The paper now uses collision-free report/quotient notation, states the
